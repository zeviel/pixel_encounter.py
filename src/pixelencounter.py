...
class PixelEncounter:
    def __init__(self):
        ...
        self.api_v2 = "https://app.pixelencounter.com/api/v2"
    ...
    def get_monster(self, monster_id: int, size: int = 512):
        response = get(
            f"{self.api}/basic/monsters/{monster_id}/png?size={size}",
            headers=self.headers).content
        return self.save_file(response)

    def get_random_monster(self, size: int = 512):
        response = get(
            f"{self.api}/basic/monsters/random/png?size={size}",
            headers=self.headers).content
        return self.save_file(response)

    def get_monsters_list(self):
        return get(f"{self.api}/basic/monsters", headers=self.headers).json()

    def get_monster_info(self, monster_id: int):
        return get(
            f"{self.api}/basic/monsterdetails/{monster_id}",
    ...
    def get_monster_svg(
            ...
            saturation: int = 1):
        url = f"{self.api_v2}/basic/svgmonsters/image/png?colored={colored}&size={size}"
        if background_color:
    ...
    def get_random_planet(
            ...
            height: int = 1080,
            frame: int = None):
        url = f"{self.api}/basic/planets?width={width}&height={height}"
        if frame:
            url += f"&frame={frame}"
        response = get(url, headers=self.headers).content
        return self.save_file(response)

    def get_planet(self, seed_id: int):
        response = get(
            f"{self.api}/basic/planets/{seed_id}",
            headers=self.headers).content
        return self.save_file(response)
